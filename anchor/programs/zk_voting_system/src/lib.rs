...
mod state;
mod error;
declare_id!("HfCyejxgjizCmDVDei3Yg7BVvT5BeChCNTVzbTThWWFP");

#[program]
pub mod zk_voting_system {
    use super::*;

    pub fn init_election(ctx: Context<InitElection>, name: String, start_time:i64, end_time:i64, options: Vec<String>) -> Result<()> {
        return init_election_handler(ctx,name,start_time,end_time, options);
    }
